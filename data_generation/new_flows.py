...
def generate_cfd_kolmogorov_flow(n_timesteps: int,
    ...
    # total wall-clock simulation time (needed but we ignore t_end later)
    ...
    # Noise initial condition (new API)
    params.init_fields.type = "noise"
    params.init_fields.noise.length = ly / kf  # small random seed
    ...
    sim = Simul(params)
    sim.state.statephys_from_statespect()  # make sure physical arrays exist
    ...
    if plot_series:
        ...
        for ax, it_snap in zip(axs, idx):
            ...
            ax.quiver(X[::stride, ::stride], Y[::stride, ::stride],
                      ...
                      color="white", scale=50, width=0.001, pivot="mid")
